...
async function sendDemo(){const inp=document.getElementById('v-input'),btn=document.getElementById('v-btn'),text=inp.value.trim();if(!text)return;if(demoCount>=maxDemo){addB(mc,'system','Demo limit reached. <a href="/signup" style="color:#ea580c">Sign up free</a>');return}inp.value='';btn.disabled=true;demoCount++;addB(mc,'out-blue',text);addB(mo,'system','<span class="spinner"></span> Reading...');try{const r=await fetch('/demo/classify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history})});const d=await r.json();lastData=d;if(mo.lastChild)mo.lastChild.remove();const reply=d.auto_reply||'Thanks for letting us know.';const cat=(d.category||'general').replace(/_/g,' ');const concern=d.concern||'';history.push({customer:text,reply});if(history.length>6)history.shift();await new Promise(r=>setTimeout(r,250));addB(mc,'in',reply);await new Promise(r=>setTimeout(r,350));const t=new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'});if(d.tier===1){const concern_html=concern?'<div style="margin:8px 0">Concern: '+concern+'</div>':'';const msg='<div style="font-weight:600;margin-bottom:6px">🚨 URGENT ('+t+')</div>Category: '+cat+concern_html+'<div style="margin:8px 0"><strong>Customer:</strong><br>'+text+'<br><br><strong>Reply:</strong><br>'+reply+'</div><div style="margin-top:12px;display:flex;gap:8px"><button style="padding:4px 10px;background:#ea580c;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">REPLY</button><button style="padding:4px 10px;background:#f0f0f0;color:#666;border:1px solid #e0e0dc;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">CLOSE</button><button style="padding:4px 10px;background:#f0f0f0;color:#666;border:1px solid #e0e0dc;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">MENU</button></div>';addB(mo,'alert-red',msg,1)}else if(d.tier===2){const concern_html=concern?'<div style="margin:8px 0">Concern: '+concern+'</div>':'';const msg='<div style="font-weight:600;margin-bottom:6px">⚠️ ISSUE ('+t+')</div>Category: '+cat+concern_html+'<div style="margin:8px 0"><strong>Customer:</strong><br>'+text+'<br><br><strong>Reply:</strong><br>'+reply+'</div><div style="margin-top:12px;display:flex;gap:8px"><button style="padding:4px 10px;background:#ea580c;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">REPLY</button><button style="padding:4px 10px;background:#f0f0f0;color:#666;border:1px solid #e0e0dc;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">CLOSE</button><button style="padding:4px 10px;background:#f0f0f0;color:#666;border:1px solid #e0e0dc;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600">MENU</button></div>';addB(mo,'alert',msg,2)}else if(d.tier===3){const concern_html=concern?'<div style="margin:4px 0">Concern: '+concern+'</div>':'';const msg='<div style="font-weight:600;margin-bottom:6px">ℹ️ FEEDBACK ('+t+')</div>Category: '+cat+concern_html;addB(mo,'feedback',msg,3)}else{const msg='<div style="font-weight:600;margin-bottom:6px">✓ LOGGED ('+t+')</div>'+cat;addB(mo,'info',msg,4)}}catch(e){if(mo.lastChild)mo.lastChild.remove();addB(mo,'system','Error: '+e.message)}btn.disabled=false;inp.focus()}
