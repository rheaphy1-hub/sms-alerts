...
def admin_ui(request: Request):
  ...
  <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:28px">
    {"".join(
      ...
      for slug, label in [("laundromat","Laundromat"),("selfstorage","Self Storage"),("mhc","Mobile Home Park"),("gym","Gym"),("carwash","Car Wash"),("rvpark","RV Park"),("hospitality","Hospitality"),("","Other / Direct")]
    )}
  </div>'''
...
VERTICALS_EXAMPLES = {
    ...
    "hospitality": ['"AC not working in 214"', '"Key card won\'t open my room"', '"No hot water"'],
}
...
<div class="nav-links"><a href="/">Demo</a><a href="/how-it-works">How It Works</a><div class="dropdown"><a href="/industries">Who We Support</a><div class="dropdown-menu"><div class="dropdown-menu-inner"><a href="/laundromat">Laundromat</a><a href="/selfstorage">Self Storage</a><a href="/mhc">Mobile Home Parks</a><a href="/gym">24/7 Gym</a><a href="/carwash">Car Wash</a><a href="/rvpark">RV Parks</a><a href="/hospitality">Hospitality</a></div></div></div><a href="/resources">Resources</a><a href="/signup" class="signup-btn">Sign Up</a></div></nav><script>(function(){document.addEventListener('click',function(e){var a=e.target.closest?e.target.closest('.dropdown>a'):null;if(a&&window.innerWidth>600&&window.matchMedia('(hover:none)').matches){var d=a.parentNode;if(!d.classList.contains('open')){e.preventDefault();document.querySelectorAll('.dropdown.open').forEach(function(o){if(o!==d)o.classList.remove('open')});d.classList.add('open');return}return}if(!e.target.closest||!e.target.closest('.dropdown')){document.querySelectorAll('.dropdown.open').forEach(function(o){o.classList.remove('open')})}},true);})();</script>"""
...
VERTICAL_HOSPITALITY_HTML = _make_vertical_page(
    slug="hospitality",
    label="Hospitality",
    headline="The AC dies at 2am. Your guest has no one to call. You find out in the review.",
    sub="Hotline gives every guest a direct line to your team — so room failures, access problems, and after-hours issues get reported while you can still fix them.",
    scenarios=[
        "Water is pouring through the ceiling in room 214",
        "My key card won\'t open my room and no one is at the desk",
        "There\'s no hot water in room 108",
        "The AC in my room has been dead since check-in",
        "Smoke alarm is chirping and I can\'t sleep",
        "The pool gate is propped open and anyone can walk in",
        "Elevator is stuck between floors",
        "Could we get extra towels sent up?",
    ],
    placements=["Guest room", "Front desk", "Elevator lobby", "Pool / fitness area"],
    step1=("Display your Hotline", "A card in each room, one at the front desk, one by the elevator. Guests always have a way to reach you."),
    step2=("Guests text instead of hunting for a person", "No app. No walking down to a desk that isn\'t staffed. They text, and Hotline reads and tiers every message."),
    step3=("You hear about it before the review does", "Flooding and lockouts hit your phone instantly. Towel requests go to your digest. Fix it while the guest is still on property."),
    plural="Hotels &amp; Hospitality"
)
...
<div class="ind-pill" onclick="setIndustry('hospitality',this)">Hospitality</div>
</div>
...
<a href="/hospitality" class="v-card"><h3>Hospitality</h3><p>After-hours guest issues, room failures, key card access</p><span class="v-cta">See it &rarr;</span></a>
</div>
...
@app.get("/hospitality")
def vertical_hospitality(): _ensure_init(); return Response(content=_ga(VERTICAL_HOSPITALITY_HTML), media_type="text/html")

@app.get("/hotel")
def vertical_hotel_redirect(): return RedirectResponse(url="/hospitality", status_code=301)
