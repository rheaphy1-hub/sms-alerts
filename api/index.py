...
def debug_env():
    ...
    return {
        ...
        "RESEND_API_KEY": _mask(os.getenv("RESEND_API_KEY","")),
        "SENDGRID_API_KEY": _mask(os.getenv("SENDGRID_API_KEY","")),
        "DIGEST_FROM_EMAIL": os.getenv("DIGEST_FROM_EMAIL","MISSING"),
        "SIGNUP_NOTIFY_EMAIL": os.getenv("SIGNUP_NOTIFY_EMAIL","MISSING"),
        "CRON_SECRET": _mask(os.getenv("CRON_SECRET","")),
...
def _process_customer_message(biz, sender, body, image_url=""):
    ...
    try:
        ...
        # Operator is mid-conversation with this customer. Their follow-up must reach the
        # operator no matter what tier it lands in — an answer like "Joe Smith, 123 Main St"
        # classifies Tier 4 and would otherwise be logged silently, breaking the thread.
        # Tier 1 falls through to the normal urgent alert (escalation still wins).
        relay_only = convo_active and tier != 1

        _trial_ok = can_send_alerts(biz)
        if relay_only and _trial_ok and alert_phones:
            ...
            logger.info(f"[CONVO RELAY SKIPPED] biz={biz['id']} trial_ok={_trial_ok} phones={len(alert_phones)}")
        elif not _trial_ok and tier != 1:
...
function resetDemo(){
  ...
  demoCount=0;history=[];replyMode=false;lastData=null;lastReplyTime=null;
